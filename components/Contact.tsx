      ...
      >
        {drop && <>
          <div className="max-w-4xl mx-auto  text-gray-300 absolute flex backdrop-brightness-[0.2] h-screen items-center justify-center flex-col z-50  space-y-10 ">
            <h1 className="text-lg font-[800] bg-gradient-to-r from-[#25D366] via-gray-400 to-[#25D366]  bg-clip-text  text-transparent"> {privacy.title} </h1>
            <div className="px-10 md:px-16 text-sm ">
            <p className="md:pb-10 pb-5"> {privacy.disc} </p>
            <ul className=" list-disc space-y-3">
              <li>When you submit a message through my contact form, I collect the following information(Name , Email , Message ) </li>
              <li>I use the collected information to respond your inquiries or messages</li>
              <li>I do not sell, trade, or otherwise transfer your personal information to third parties without your consent.</li>
            </ul>
            </div>
            <button className=" z-50 px-4 py-1 text-[#25D366]/80 text-sm hover:border border-[#25D366] hover:pointGreen  rounded-full   bg-[linear-gradient(#ffffff99,#ffffff00,#0000004d,#ffffff33)] before:block before:contents-['']  before:absolute before:left-[8px] before:right-[8px] before:top-[5px] before:h-[10px] before:transform  before:rounded-full before:bg-[linear-gradient(#ffffffcc,#ffffff00)]   before:opacity-30" 
            onClick={() => setDrop(false)}>close</button>
          </div>
        </>}
      </AnimatePresence>
        ...
        <div className=" overflow-hidden relative w-full md:w-[25rem] mx-auto mt-8 lg:m-0 filter  bg-black pointGreen  border-2 border-[#25D366] mix-blend-screen   rounded-2xl p-8">
          ...
          <motion.div
            ...
            {/*
              ...
              */}
            <SocialIcon url="https://www.linkedin.com/in/deaa-aldin-alawad/"
          ...
          <form
            ...
            <div className="flex flex-row space-x-2 py-[18px] ">
              <input
                ...
                id='checkbox'
              />
              <label className="text-white text-sm font-semibold">I agree to the <button type="button" onClick={() => setDrop(true)} className="text-[#25D366]/40 font-[800] cursor-pointer hover:text-[#25D366] text-center pl-1">privacy policy</button></label>
            </div>

            {/*submit button */}
            <button className=" z-0 transition-all duration-150  font-bold link animate-puls 
