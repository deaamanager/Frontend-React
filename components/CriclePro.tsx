...
function CriclePro({ percantge, circlewidth }: any) {
  ...
  useEffect(() => {
    ...
    if (isVisible === true) {
      ...
      animationTimeout = setTimeout(() => {
        refCi.current?.style.setProperty(
          "strokeDasharray",
          dashArray.toString()
        );
        refCi.current?.style.setProperty(
          "strokeDashoffset",
          dashoffset.toLocaleString()
        );
  ...
  return (
    <div ref={ref}>
      <svg
        className="pointGreen bg-gray-900/10 rounded-full  w-20 h-20
      ...
      >
        ...
        <text
          ...
          dy="0.3em"
          textAnchor="middle"
          className={`fill-[#25D366] font-bold text-[2.5rem]`}
        >
          {percantge}%
        </text>
      </svg>
